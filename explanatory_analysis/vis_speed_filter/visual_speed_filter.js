// The purpose of this script is to only show the result in visual blocks in a specific speed. 
// For exaple we want to know how people performed in visual trials where rhythms were presented at 250 ms vs how they performed in visual 270 ms or 230 ms.
// Line 9 (speedFilter) needs to be adjusted considering which speed we want. 1 = 230 ms, 2 = 250 ms, 3 = 270 ms.
var XLSX = require('xlsx');
const fs = require('fs');
var excel = require('excel4node');
var finalExcel = [];

const speedFilter = 3;  // Adjust the value between 1, 2, or 3 to get result in specific tempo.
function processFile(fileName) { //all the process of finding correct answers, attention check failures etc. are done here
    ...
    function parseVisReg(sheet) { 
        ...
        Object.keys(sheet).forEach(element => {
            ...
            var row = splited[1];
            if(sheet[`${speedColumn}${parseInt(row)}`]?.v != speedFilter){
                return;
            }
            if (column === seqColumn) {
                sheet["AC1699"]
        ...
        Finalized['VisReg'] = {
            ...
            Wrong: wAnswerCount,
            Score: parseFloat((cAnswerCount / 7).toFixed(2)),
    ...
    fs.writeFileSync(`../../data/json_vis270_Only/Processed ${formatFileName(fileName)}.json`, JSON.stringify({ ...Finalized }), (err) => { //creates JS files
...
function processJson(fileName) {
    ...
    var file = require(`../../data/json_vis270_Only/${fileName}`);
...
function makeExcel(finalExcel){
    ...
    worksheet.cell(1,2).string('VisSD');
    worksheet.cell(1,3).string('VisIrreg');
    ...
    worksheet.cell(1,6).string('VisSD');
    worksheet.cell(1,7).string('VisIrreg');
    worksheet.cell(1,8).string('VisReg');
    ...
    worksheet.cell(1,10).string('VisSDIntDiff (T)');
    worksheet.cell(1,11).string('VisSDIntDiff (F)');
    worksheet.cell(1,12).string('VisIrregIntDiff (T)');
    worksheet.cell(1,13).string('VisIrregIntDiff (F)');
    worksheet.cell(1,14).string('VisRegIntDiff (T)');
    worksheet.cell(1,15).string('VisRegIntDiff (F)');
    ...
    worksheet.cell(1,17).string('VisSD Index');
    worksheet.cell(1,18).string('VisIrreg Index');
    worksheet.cell(1,19).string('VisReg Index');
    ...
    Object.keys(finalExcel).forEach(key=>{
        ...
        worksheet.cell(x,6).number(finalExcel[key][3]);
        ...
        worksheet.cell(x,10).number(finalExcel[key][6]);
        worksheet.cell(x,11).number(finalExcel[key][7]);
        ...
        worksheet.cell(x,17).number(finalExcel[key][12]);
        worksheet.cell(x,18).number(finalExcel[key][13]);
        worksheet.cell(x,19).number(finalExcel[key][14]);
    ...
    workbook.write("../../results/Visual_270_Only.xlsx"); // Change 230 to 250 or 270 according to the tempo of interest
}
...
function makeFinalExcelFromJsons(){
    fs.readdir('../../data/json_vis270_Only', (err, files) => {
        files.forEach(file => {
...
// makeJsonFromXlsx(); // Converts XLSX files From `data/2_preprocessed_data` folder to seperate JSON files into `data/json_vis230_Only` folder. 230 can change to 250 or 270 depending on the tempo of interest.
// This function sould run before makeFinalExcelFromJsons function.
makeFinalExcelFromJsons(); // Converts JSON files form data folder into single Excel file (Visual_230_Only.xlsx). This function should not run without having JSON files in data folder.

// Note: a folder named json_vis270_Only should be created inside data folder before running the code. 