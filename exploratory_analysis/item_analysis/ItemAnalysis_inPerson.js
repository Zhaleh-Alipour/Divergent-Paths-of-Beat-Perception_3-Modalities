...
(() => {
    fs.readdir('../data/3_json_files', (err, files) => {
        ...
        seqs.forEach((s, index) => {
            ...
            files.forEach(fileName => {
                ...
                if (fileName.endsWith('json')) {
                    var file = require(`../data/3_json_files/${fileName}`);
                    var participantNumber = fileName.split("Processed Preprocessed")[1].split(".json")[0].split(" ")[1]; //Processed Preprocessedmain
                    if (!ScoreSet) {
                        if (mod === "VisReg") {
        ...
        workbook.write("../results/Item_Analysis.xlsx");
    });
})();
