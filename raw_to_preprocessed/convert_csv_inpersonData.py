# This code is for getting raw data files that are in a folder called "1_raw_data", keeping necessary columns, renaming them, and removing the rest.
# At the end it saves files in a new folder called "2_preprocessed_data". We need to create this folder ourselves before running the code.
# We will end up having 45 columns.
from pathlib import Path
import pandas as pd
# b is the columns of the raw data file we wanna keep. At the left side of : sign is the name of column and the right of : sign is the name we wanna
...
print("CSV count:", len(list(DATA_DIR.rglob("*.csv"))))
...
for csv in DATA_DIR.rglob('*.csv'):
    print(csv)
    df = pd.read_csv(csv)
    result = df[['participant', 'date', 'expName', 'SDaud', 'diff_sd', 'main_key_resp_audSD.keys', 'key_resp_16.keys', 'SDdeviantPitch',
                 'IrregAudSequence', 'diff_irreg', 'main_key_resp_audIrreg.keys', 'key_resp_6.keys', 'IrregDeviantPitch', 'RegAudSequence',
                 'diff_reg', 'main_key_resp_audReg.keys', 'keyACaudreg.keys', 'RegDeviantPitch', 'SDinterval', 'SDspeed', 'SDDiff',
                 'main_key_resp_visSD.keys', 'SDColor', 'irregNumberOfIntervals', 'irregSpeed', 'irregDiff', 'main_key_resp_visIrreg.keys',
                 'irregColor', 'NumberOfIntervals', 'Speed', 'Diff', 'main_key_resp_visReg.keys', 'Color', 'TactSD1', 'TactSD-Speed', 'TactSD_Diff',
                 'key_resp_34.keys', 'TactIrreg_NumberOfIntervals','SpeedTactIrreg3', 'Tact_irreg_Diff', 'key_resp_30.keys', 'Tact-NumberOfIntervals',
                 'tact_Speed', 'Tact_Diff', 'key_resp_26.keys']]
